...
static const struct regval imx586_linear_10bit_1920x1080_30fps_nopd_regs[] = {
   /*mipi output setting*/
        {0x0112,0x0A},
        {0x0113,0x0A},
        {0x0114,0x03},// MIPI 2lane 0x01, 4lane 0x03

	
	/*line length pck setting*/ 
		{0x0342,(7530>>8)&0xff},//0x1e  30 24
		{0x0343,7530&0xff},//0x87  91,0xA1  88
	/*frame length lines setting*/
		{0x0340,(2280>>8)&0xff},//0xe    09
		{0x0341,2280&0xff},//0xF4 0x68	0c
	...
	/*mode setting binning V2H2*/
		{0x0220,0x62},
		{0x0222,0x01},
        {0x0900,0x01},
        {0x0901,0x44},//4binning 1080P
        {0x0902,0x08},
        {0x3140,0x00},
        {0x3246,0x89},
        {0x3247,0x89},
    ...
    /*clock setting*/    
        ...
        {0x0305,0x04},
        {0x0306,0x01},
        {0x0307,0x68},//PLL MPY 360
        {0x030B,0x02},
        {0x030D,0x02},//OPSCLK preDiv, OPSCLK=1200M=24*200/2/2.
        {0x030E,0x00},
        {0x030F,200},     //0x0104
        {0x0310,0x01},
			       ...
			       struct rkmodule_inf *inf)
{
	...
	if (otp->lsc_data.flag) {
		inf->lsc.flag = 1;
		inf->lsc.lsc_w = otp->basic_data.size.width;
		inf->lsc.lsc_h = otp->basic_data.size.height;
		inf->lsc.decimal_bits = otp->lsc_data.table_size;
...
static int imx586_open(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
	...
	const struct imx586_mode *def_mode = &supported_modes[0];     //chose case
